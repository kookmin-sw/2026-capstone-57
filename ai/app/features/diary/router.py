...
[규칙]
1. 질문은 1개만 생성하세요.
2. 이전 답변에서 언급된 구체적인 사건이나 에피소드를 더 깊이 탐색하세요.
3. 일정에 있는 활동뿐 아니라, 일정 사이 빈 시간이나 수업 전후에 뭘 했는지도 물어볼 수 있습니다.
4. 이미 물어본 내용을 반복하지 마세요.
5. 친근하고 부담 없는 반말 해요체를 사용하세요.
6. 현재 {current_turn}/{max_turns} 턴입니다.
7. 사용자가 충분히 풍부한 답변을 했다고 판단되면, 질문 대신 정확히 "[COMPLETE]"만 출력하세요.
8. 아직 더 물어볼 내용이 있다면 질문을 생성하세요.
9. 오늘 실제로 한 일, 만난 사람, 있었던 일에 초점을 맞추세요.
"""
