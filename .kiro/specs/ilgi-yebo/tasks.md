...
- [ ] 5. 매칭 서비스 구현
  ...
  - [ ] 5.2 매칭 점수 계산 엔진 구현
    ...
    - _요구사항: 4.9, 4.10, 4.16, 4.17, 4.18, 11.3_
  ...
  - [ ]* 5.3 Property 14, 26 속성 테스트: 매칭 점수 및 차단 필터
    ...
    - **검증 대상: 요구사항 4.9, 4.10, 4.16, 4.17, 4.18, 11.3**

  - [ ] 5.4 배치 매칭 실행 로직 구현
    - `executeBatchMatching`: 평일(월~금) 자정 배치 매칭 전체 흐름 (Spring @Scheduled 또는 Spring Batch)
    - 빈 슬롯 필터링, 활성 매칭 5일 주기 건너뛰기
    ...
    - `respondToMatchCycleExtension`: 매칭 주기 연장 동의/거부 처리 (양쪽 동의 시 cycle_end_date +5일 연장(다음 주 금요일까지), 거부 시 기존 주기 유지)
    - _요구사항: 4.1, 4.2, 4.3, 4.4, 4.5, 4.6, 4.7, 4.11, 4.12, 4.13, 4.14, 4.15, 4.16, 4.17, 4.18, 13.2, 13.4_
  ...
  - [ ]* 5.5 Property 11, 12, 15, 30, 41 속성 테스트: 배치 매칭 규칙
    ...
    - **검증 대상: 요구사항 4.2, 4.3, 4.4, 4.5, 4.6, 4.7, 4.11, 4.13, 4.14, 4.15, 12.1, 13.2, 13.4**
...
- [ ] 15. AI/LLM 서비스 구현 (Amazon Bedrock)
  - [ ] 15.1 AIService 기본 구조 구현
    ...
    - _요구사항: 2.6, 4.10, 5.1, 8.1, 9.2, 9.3, 14.6_
...
- [ ] 17. 서비스 간 통합 및 배치 스케줄러 연결
  - [ ] 17.1 배치 매칭 스케줄러 구현
    - Spring @Scheduled(cron = "0 0 0 * * MON-FRI") 평일 자정 실행 크론 작업 설정
